const {setting_hari} = require("../model");

module.exports = {
    async tampil (req, res, next){
      try {
        await setting_hari.findAll().then(result => {
            if (result.length > 0) {
    ...
    async cari (req, res, next){
        try {
            await setting_hari.findOne({where : {
    ...
    async edit (req,res,next){
        try {
            const {id_wilayah,id_shif,nama_hari,status_hari,status} = req.body;
            await setting_hari.update({
                id_wilayah,
                id_shif,
                nama_hari,
                status_hari,
                status
            },{where : {id_set_hari : req.params.id_set_hari}}).then(result => {
                if(result == 1){
                    return res.status(201).json({
                        success: 1,
